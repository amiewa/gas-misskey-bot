...
function processScheduledPost() {
  ...
  for (let i = 1; i  data.length; i++) {
     時間帯指定（例 7 や 7,8 や 7-9 などのパースが必要だが、ここでは簡易的に完全一致かカンマ区切り）
  ...
  if (config.ENABLE_EVENT_POST) {
    const todayStr = Utilities.formatDate(now, 'JST', 'MMdd');
...
function processGeminiPost() {
  ...
  try {
    ...
    if (!texts) return;
     ...
     2. キーワード抽出 (API節約のため1回で済ませる工夫も可だが、計画書通り2段階で実装)
    const extractPrompt = `以下のテキスト群から、現在話題になっている特徴的な名詞やテーマを3つ抽出してください。単語のみをカンマ区切りで返してください。nn${texts}`;
    const keywords = callGemini(extractPrompt);

     3. 文章生成
    const systemPrompt = getSystemPrompt();
    const genPrompt = `${systemPrompt}nn以下のキーワードを使って、Misskeyに投稿する140文字程度の雑談を作ってください。キーワード ${keywords}`;
...
function processPollPost() {
  ...
  if (!config.ENABLE_POLL_POST) return;
   ...
   TL取得と簡易ワード抽出(正規表現で名詞っぽいものを拾う簡易版)
...
function processReaction() {
  ...
  const candidates = timeline.filter(n = {
    ...
    return noteTime  thresholdTime && 
           !n.user.isBot && 
           n.userId !== config.MISSKEY_USER_ID;  自分のID判定が必要ならconfigに追加
  });
  ...
  callMisskeyApi('notesreactionscreate', { noteId target.id, reaction reaction });
