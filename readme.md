...
2. 一番下の `BOT_ACTIVE` を **`TRUE`** に書き換えます。
3. 最大5分待つか、Apps Script画面から `mainDispatcher` 関数を手動で実行してみます。
4. Misskeyのホームタイムラインに投稿されれば成功です！
5. 動作に問題がなければ、スプレッドシートの「設定」シートにある **`POST_VISIBILITY`** を `home` から `public` に書き換えてください。これで誰からでも見えるようになります。
